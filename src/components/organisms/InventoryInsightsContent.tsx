...
import { InventoryInsightsHeader, InventoryStatsGrid, InsightsBarChart, InventoryStudentList } from "@/components/molecules";
import { TrendingUp, Users, BarChart3 } from "lucide-react";
import type { InventoryInsightData } from "@/types/inventory-insights";

export const InventoryInsightsContent: React.FC = () => {
  ...
  useEffect(() => {
    if (type && ["mental-health-prediction", "bmi-category", "physical-health"].includes(type)) {
      fetchInsights(type as "mental-health-prediction" | "bmi-category" | "physical-health");
    } else {
      navigate("/dashboard");
    }
  }, [type, fetchInsights, navigate]);

  const handleBarClick = (data: InventoryInsightData) => {
    if (canDrillDown) {
    ...
    <div className="flex-1 flex flex-col bg-gray-50">
      ...
      <div className="flex-1 p-4 sm:p-6">
        <div className="max-w-7xl mx-auto">
          {/* Stats Grid - Only show at overview level */}
          {insights.currentLevel.level === "overview" && <InventoryStatsGrid />}
          ...
          {isStudentView ? (
            <InventoryStudentList
              ...
              loading={loading}
              title={`Students - ${insights.filters.program} ${insights.filters.yearLevel} ${insights.filters.gender}`.trim()}
            />
          ) : (
            <>
              {/* Summary Cards */}
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
                <div className="bg-white rounded-lg border shadow-sm p-4 sm:p-6">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center text-blue-600 bg-blue-50 rounded-lg flex-shrink-0">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xl sm:text-2xl font-bold text-gray-900">
                        {insights.currentLevel.data.length > 0
                          ? Math.round(totalCount / insights.currentLevel.data.length)
                          : 0}
                      </p>
                      <p className="text-xs text-gray-500 mt-1 truncate">Mean distribution</p>
                    </div>
                    <div className="w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center text-green-600 bg-green-50 rounded-lg flex-shrink-0">
                ...
                <div className="bg-white rounded-lg border shadow-sm p-4 sm:p-6 sm:col-span-2 lg:col-span-1">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center text-purple-600 bg-purple-50 rounded-lg flex-shrink-0">
                                ...
                                <tr key={index} className="hover:bg-gray-50">
                                  <td className="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap">
                                    ...
                                    <div className="sm:hidden text-xs text-gray-500 mt-1">
                                      {percentage}% ({item.value} students)
                                    </div>
                                  </td>
